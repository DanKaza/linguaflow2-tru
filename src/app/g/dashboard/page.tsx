...
export default function TeacherDashboard() {
  ...
  // Data demo — langsung dari demo-data (tanpa query).
  const classes: TeacherClass[] = getDemoClassesByTeacher(teacherId).map((c) => ({
    id: c.id,
    name: c.name,
    code: c.code,
  ...
  const activeTasks = DEMO_TASKS.filter(
    (t) => t.teacher_id === teacherId && t.deadline >= todayKey,
  ).length;
